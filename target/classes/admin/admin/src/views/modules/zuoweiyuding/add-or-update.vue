<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="学生号" prop="xueshenghao">
          <el-input v-model="ruleForm.xueshenghao" 
              placeholder="学生号" clearable  :readonly="ro.xueshenghao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="学生号" prop="xueshenghao">
              <el-input v-model="ruleForm.xueshenghao" 
                placeholder="学生号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          	            if(o=='xueshenghao'){
            this.ruleForm.xueshenghao = obj[o];
	    this.ro.xueshenghao = true;
            continue;
          }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
                      		if(json.xueshenghao!=''&&json.xueshenghao){
              		this.ruleForm.xueshenghao = json.xueshenghao
		}
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `zuoweiyuding/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zuoweiyudingCrossAddOrUpdateFlag = false;
                  this.parent.search();
